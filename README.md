...
./.venv/Scripts/python -m pytest tests/ -v   # 828 tests
...
- **Resource ceilings**: max WebSocket frame size, max ops per message, max rooms per server process, max clients per room — every one env-tunable, every violation closes the connection with a distinct, documented code rather than a silent drop.
