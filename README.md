...
./.venv/Scripts/python -m pytest tests/ -v   # 763 tests
...
All reachable from the UI: an explicit **Save**, per-format download buttons, and (2D) an **Import SVG/DXF** picker that broadcasts the imported paths to everyone in the room.
...
**2D sketch** (`/2d`): pen and shape tools (Line/Rect/Circle/Ellipse/Arc with live numeric dimension input), a select tool with multi-select, transform, duplicate, copy/paste, align/distribute, and object snapping, a persistent constraint system (coincident/parallel/perpendicular/fixed-distance/tangent, re-solving automatically on drag), Measure and Dimension tools, text/fills/stroke styles/groups, a real pan/zoom/grid viewport with document units (px/mm/in), live multi-user cursors and presence, comments, undo/redo, and export to JSON/SVG/DXF/PNG.
